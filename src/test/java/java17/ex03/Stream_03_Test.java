...
public class Stream_03_Test {
  ...
  public void test_flatMap_distinct() throws Exception {
    ...
    List<Pizza> result =
        orders.stream().flatMap(order -> order.getPizzas().stream()).distinct().toList();
  ...
  public void test_grouping() throws Exception {
    ...
    Map<Customer, List<Order>> result =
  ...
  public void test_partitionning() throws Exception {
    ...
    Map<Boolean, List<Pizza>> result =
        pizzas.stream().collect(Collectors.partitioningBy(p -> p.getName().startsWith("L")));
