...
public class Stream_01_Test {
  ...
  public void test_stream_filter() throws Exception {
    ...
    List<Pizza> result =
        pizzas.stream().filter(p -> p.getPrice() >= 1300).collect(Collectors.toList());
  ...
  public void test_stream_findFirst() throws Exception {
    ...
    Optional<Order> result =
        orders.stream().filter(o -> o.getCustomer().getFirstname().equals("Sophie")).findFirst();
  ...
  public void test_stream_filter_adv() throws Exception {
    ...
    List<Order> result =
        orders.stream()
            .filter(o -> o.getPizzas().stream().anyMatch(p -> p.getName().equals("Pépéroni")))
